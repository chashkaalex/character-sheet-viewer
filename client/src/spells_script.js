...
function renderSpellsUI() {
    ...
    if (!character || !character.spellCasting || !character.spellCasting.classSpellCastingData) {
        container.innerHTML = '<div>No spellcasting available.</div>';
        return;
    }

    const spellCastingData = character.spellCasting.classSpellCastingData;
...
function generateSpellOptions(className, maxLevel, isDomain, selectedLevels = null) {
    ...
    const casterClassData = character.spellCasting.classSpellCastingData.find(c => c.className === className);
...
function getCurrentSpellcastingClass() {
    ...
    if (character && character.spellCasting && character.spellCasting.classSpellCastingData) {
        return character.spellCasting.classSpellCastingData[0]?.className || 'Cleric';
    }
    return 'Cleric';
}
...
function prepareSpell() {
    ...
    google.script.run
        ...
        .OnPrepareSpell(character.docId, slotData, selectedSpell);
...
function castSpell() {
    ...
    google.script.run
        ...
        .OnCastSpell(character.docId, slotData);

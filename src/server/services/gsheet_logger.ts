 ...
 */
export function recordUserLog(functionName: string, message: string, type: 'INFO' | 'ERROR' = 'INFO'): void {
  try {
    ...
    if (sheet) {
      ...
      console.error(`ERROR: Sheet "${LOG_SHEET_NAME}" not found in spreadsheet.`);
    }
...
// GAS environment compatibility
(globalThis as any).recordUserLog = recordUserLog;

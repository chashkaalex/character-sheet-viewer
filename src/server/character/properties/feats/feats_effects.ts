import { EffectData, MutatingEffectData, StaticPropertyEffect } from '../../state/effects';
import { ICharacter } from '../../icharacter';
import { Armor } from '../../gear/items/items';

export const StatusRelatedValue = (character: ICharacter, statusName: string, value: number): number => {
 ...
 */
export const FeatEffects: Record<string, EffectData[]> = {
  'Absolute Steel (Stance)': [
    {
      status: 'Absolute Steel (Stance)',
      description: 'While you are in this stance, you gain a +10-foot enhancement bonus to your speed. If you move at least 10 feet during your turn, you gain a +2 dodge bonus to AC until the beginning of your next turn.',
      callback: (character: ICharacter) => {
        if (!character.actions.includes('Absolute Steel Stance')) {
          character.actions.push('Absolute Steel Stance');
        }
      }
    }
  ],
  ...
  'Heavy Armor Optimization': [
    {
      ...
      valueResolver: (character: ICharacter) => {
        const hasHeavyArmor = character.battleGear.some(item => item instanceof Armor && item.armorType === 'Heavy');
        return hasHeavyArmor ? 1 : 0;
      }
    },
    {
      status: 'Heavy Armor Optimization',
  ...
  'Combat Casting': [
    {
      ...
      modifierType: 'Generic',
      valueResolver: (character: ICharacter) => StatusRelatedValue(character, 'Casting Defensively', 4)
    }
  ],
  'Combat Expertise': [
    {
      status: 'Combat Expertise',
      description: 'As a standard or full attack action in melee, you can take a penalty of up to -5 on attack rolls and add the same number as a dodge bonus to Armor Class [4].',
      callback: (character: ICharacter) => {
        if (!character.actions.includes('Combat Expertise')) {
          character.actions.push('Combat Expertise');
        }
      }
    }
  ],
  ...
  'Dragon Hunter': [
    {
      ...
      property: 'ac',
      modifierType: 'Dodge',
      valueResolver: (character: ICharacter) => StatusRelatedValue(character, 'Fighting Dragon', 2)
    }
  ],
  'Dragonfoe': [
    {
  ...
  'Stunning Fist': [
    {
      status: 'Stunning Fist',
      description: 'Forces a foe damaged by your unarmed attack to make a Fortitude saving throw (DC 10 + 1/2 your character level + your Wisdom modifier) or be stunned for 1 round. Usable once per day for every four levels you have attained (Monks have additional uses) [23].',
      callback: (character: ICharacter) => {
        if (!character.actions.includes('Stunning Fist')) {
          character.actions.push('Stunning Fist');
        }
      }
    }
  ],
  ...
  'Practiced Spellcaster': [
    {
      ...
      // casterClassName is injected into args at parse time by feats_parser.ts
      callback: (character: ICharacter, args: Record<string, unknown>) => {
        const params = args['params'] as string[] | undefined;
        const casterClassName = params?.[0];
        if (!casterClassName) {
          character.parseWarnings.push('Practiced Spellcaster: casterClassName not resolved');
          return;
        }
        const casterLevel = character.spellCasting.GetCasterLevel(casterClassName);
        const classLevel = character.GetClassLevel(casterClassName);
        const bonus = (classLevel + 4 >= character.HD) ? (character.HD - classLevel) : 4;
        const effect = new StaticPropertyEffect({
          status: 'Practiced Spellcaster',
          property: `casterLevel ${casterClassName}` as any,
          value: bonus,
          modifierType: 'Competence'
        });
        casterLevel.applyEffect(effect);
      }
    } as MutatingEffectData
  ],
  'Quick Draw': [
  ...
  'Shield Specialization (Tower)': [
    {
      ...
      modifierType: 'Generic',
      valueResolver: (character: ICharacter) => {
        const hasTowerShield = character.battleGear.some(item =>
  ...
  'Song of the Heart': [
    { status: 'Song of the Heart', property: 'Inspire Courage', modifierType: 'Generic', value: 1 },
    { status: 'Song of the Heart', property: 'Inspire Competence', modifierType: 'Generic', value: 1 },
    { status: 'Song of the Heart', property: 'Inspire Greatness', modifierType: 'Generic', value: 1 },
    { status: 'Song of the Heart', property: 'Inspire Heroics', modifierType: 'Generic', value: 1 }
  ],
  'Spell Focus': [
  ...
  'Weapon Focus': [
    {
      ...
      callback: (character: ICharacter, args: Record<string, unknown>) => {
        const params = args['params'] as string[] | undefined;
        const weaponParam = params?.[0];
        if (!weaponParam) return;

        character.addGearEffect(
          `Weapon Focus (${weaponParam})`,
          weaponParam,
          {
            status: `Weapon Focus (${weaponParam})`,
            property: 'featAttackBonus',
            modifierType: 'Generic',
            value: 1
          }
        );
  ...
  'Weapon Specialization': [
    {
      ...
      callback: (character: ICharacter, args: Record<string, unknown>) => {
        ...
        character.addGearEffect(
          `Weapon Specialization (${weaponParam})`,
          weaponParam,
          {
            status: `Weapon Specialization (${weaponParam})`,
  ...
  'Greater Weapon Focus': [
    {
      ...
      callback: (character: ICharacter, args: Record<string, unknown>) => {
        ...
        character.addGearEffect(
          `Greater Weapon Focus (${weaponParam})`,
          weaponParam,
          {
            status: `Greater Weapon Focus (${weaponParam})`,
            property: 'featAttackBonus',
            modifierType: 'Generic',
            value: 1
          }
  ...
  'Greater Weapon Specialization': [
    {
      status: 'Greater Weapon Specialization',
      callback: (character: ICharacter, args: Record<string, unknown>) => {
        const params = args['params'] as string[] | undefined;
        const weaponParam = params?.[0];
        if (!weaponParam) return;
        ...
        character.addGearEffect(
          `Greater Weapon Specialization (${weaponParam})`,
          weaponParam,
          {
            status: `Greater Weapon Specialization (${weaponParam})`,
            property: 'featDamageBonus',
            modifierType: 'Generic',
            value: 2
          }
